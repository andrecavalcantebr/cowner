...
- GCC 10+ ou Clang 12+ (necessário para `__attribute__((cleanup))` e `__attribute__((error(...)))`)
- Padrão mínimo: `-std=gnu11` (recomendado: `-std=gnu2x`)
- Sistema operacional: Linux/macOS (requer `explicit_bzero`, disponível no glibc 2.25+ e macOS 10.12+)

### Instalação

Copie os dois arquivos para dentro do seu projeto:

```bash
# via git (recomendado — mantém rastreabilidade)
git clone https://github.com/andrecavalcantebr/cowner.git
cp cowner/src/cowner.h cowner/src/cowner.c seu-projeto/src/

# ou baixe diretamente
curl -O https://raw.githubusercontent.com/andrecavalcantebr/cowner/main/src/cowner.h
curl -O https://raw.githubusercontent.com/andrecavalcantebr/cowner/main/src/cowner.c
```

Não há dependências externas além da biblioteca C padrão.

### Integração

Adicione `cowner.c` às suas fontes e inclua o header onde necessário:

```c
#include "cowner.h"
```

Compile junto com os demais arquivos do projeto:

```bash
gcc -std=gnu2x -Wall -Wextra -o meu_prog main.c cowner.c
```
...
int main(void) {
    // UniquePtr com cleanup automático ao sair do escopo
    UNIQ_SCOPED UniquePtr p = UniquePtr_make(sizeof(int));
    if (!p.data) return 1;   // OOM

    *(int *)UniquePtr_get(&p) = 42;
    printf("%d\n", *(int *)UniquePtr_get(&p));

    // p liberado automaticamente aqui
    return 0;
}
```
...
## Para desenvolvedores do cowner
...
cowner/
├── src/
│   ├── cowner.h   — API pública (tipos, funções, macros)
│   ├── cowner.c   — Implementação (CtrlBlock privado)
│   └── main.c     — Exemplos e testes manuais
├── Makefile
├── LICENSE        — GNU GPL v3
├── README.md      — Documentação (este arquivo)
└── agent.md       — Contexto para o agente de IA
```

### Build

```bash
make          # compila em ./cowner
make clean    # remove binários e objetos
make vars     # exibe variáveis do Makefile
```

O Makefile usa `-std=gnu2x -Wall -Wextra`. Nenhum warning é aceitável.

### Verificação de memória

```bash
valgrind --leak-check=full --error-exitcode=1 ./cowner
```

Todo `alloc` deve ter um `free` correspondente. Zero leaks é o critério de aceite.

### Testando erros de compile-time

Para verificar que as operações proibidas falham corretamente:

```bash
# deve falhar com: error: call to 'UniquePtr_copy' declared with attribute error
echo '#include "src/cowner.h"
void t(void){ UniquePtr p={0}; UniquePtr_copy(&p); }' | gcc -std=gnu2x -xc - -Isrc 2>&1
```

### Convenções para contribuições

- Sem warnings com `-Wall -Wextra`
- Valgrind limpo antes de qualquer commit
- Seguir as convenções de nomenclatura descritas na seção [Convenções de nomenclatura](#convenções-de-nomenclatura)
- Novas operações proibidas devem usar `__attribute__((error(...)))`, nunca `assert`
- `abort()` + `fprintf(stderr, ...)` para erros detectáveis apenas em runtime
